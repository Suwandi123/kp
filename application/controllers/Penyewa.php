...
class Penyewa extends CI_Controller
{
	...
	public function tambah_aksi()
	{
		...
		$validation->set_rules(penyewa_form_validation());

		if ($validation->run()) {
			$input = $this->input;

			$this->M_penyewa->input_data([
				"nik" => $input->post("nik"),
				"nama" => $input->post("nama"),
				"ttl" => $input->post("ttl"),
				"alamat" => $input->post("alamat"),
				"kedusunan" => $input->post("dusun")
			]);
			...
			$alrt = "danger";
			$msg = "Silakan input kembali dengan benar !";
		}

		$this->session->set_flashdata('pesan', '<div class="alert alert-' . $alrt . '" role="alert">' . $msg . '</div>');
